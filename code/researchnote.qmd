...
cate <- readxl::read_xlsx("../data/category.xlsx") %>%
  rename(countryname = country)
cate$ccode <- countrycode(cate$countryname,
                                origin = "country.name",
                                destination = "cown",
                                 custom_match = c(
                                   "Sudan-South"= "626"))
cate$year <- as.factor(cate$year)
cate$ccode <- as.integer(cate$ccode)
...
maindata <- cate %>%
  ...
  left_join(.,vdem,by = c("ccode","year","countryname")) %>%
...
#1write.csv(fulldata,file = "../data/fulldata.csv",row.names = FALSE)
...
plot_rus <- df_rus %>%
  ...
  annotate("text", x = "1948", y = .23, label = "Stalin",
             color="#000000",size = 2,fontface="bold" ) +
...
## 
plot_full <- ggarrange(plot_taw,plot_rus,
                       ncol = 1, nrow = 2)
...
```{r test}
# group 1
eco_good <- fulldata_auth %>%
  filter(.,gdppc_mpd_log >= median(gdppc_mpd_log,na.rm = TRUE))
eco_bad <- fulldata_auth %>%
  filter(.,gdppc_mpd_log < median(gdppc_mpd_log,na.rm = TRUE))

# group 2
euq_good <- fulldata_auth %>%
  filter(.,gini_tax >= median(gini_tax,na.rm = TRUE)) 
euq_bad <- fulldata_auth %>%
  filter(.,gini_tax < median(gini_tax,na.rm = TRUE))

# group 3
regime_monarchy <- fulldata_auth %>%
  filter(.,regimeType == 4)
regime_party <- fulldata_auth %>%
  filter(.,regimeType == 1)
regime_personal <- fulldata_auth %>%
  filter(.,regimeType == 2)
regime_military <- fulldata_auth %>%
  filter(.,regimeType == 3)



worldleaders <- regime_military %>%
  select(.,countrycode,year,leader,diff_year,auth) %>%
  rename(unit = countrycode,
         time = year,
         growth = diff_year) %>%
  unique() 
